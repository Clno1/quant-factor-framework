...
2026-08-20 已把 MA20/50/60/120/200 水平和 5/20 日变化落入 Stage A，并要求每天
有效行情至少覆盖 95% 当日 PIT 成分。冻结假设和完整实现口径见
[大盘顶底研究 v2：市场宽度与 COR1M](market_regime_breadth_cor1m_v2.md)。
...
`COR1M` 是 Cboe 从 SPX 和最大 50 只成分股期权推导的一个月隐含平均相关性。
它描述预期共同波动而不是市场宽度，也不能用 VIX 替代。高位或快速上升表示分散化
收益可能消失；一月期限还受财报季影响，所以正式候选优先测试变化和滚动分位，禁止
直接写死绝对阈值。[Cboe Implied Correlation](https://www.cboe.com/us/indices/implied/)
