...
### 30.11 2026-09-13：精确版本读取隔离与XMAX单票新证据

本轮本地main修复并在SG隔离验证，未部署生产或提交/push；正式coverage/PIT/主表指针均未变。
不依赖供应商反馈、不发送工单、不自动发布、不重算八因子，不修改Cursor板块轮动代码。
297项相关及共享模块测试通过。完整实现及证据见
[本轮审计](../reviews/2026-09-13-cup-resilience/README.md)。

对30.10所述31项的准确解释：15只以有证据、有边界的别名重抓整票；16只的114个坏原始行
逐字段匹配既有认证隔离，移除有效父日期0。不是豁免31只。更早分页修复是100页上限提升200，
实际102页到达历史边界。茶杯柄合同any改为非空且all，避免部分周期不完整被放行。

新增`SecurityMasterStore.load_generation(id)`按精确已发布版本读取并验证所有原有hash/manifest；
breakout适配器确认coverage与PIT绑定ID/hash一致后加载该版本，不再强迫匹配最新主表。
因此新主表发布不会使仍完整的历史bundle失效，真实MDB v3回放已在隔离代码中成功。此修改
不让过期数据生成新候选：09-14/source09-11验证仍明确报coverage09-10 stale。

XMAX新增证据为[经纪商2025-11-10实际执行的公司行动记录](https://community.trading212.com/t/corporate-events-in-november-2025/88702/4)，
佐证正式主表11-10边界；[SEC文件](https://www.sec.gov/Archives/edgar/data/1473334/000149315225020756/form8-k.htm)
只证明公司身份/更名背景，不冒充交易所切换日公告。新增映射仅让NVFY的11-06/07通过XWIN查询，
保留历史NVFY身份。新鲜整票重抓三个价格端点、独立核对近期重叠后，1934/1934日期通过，
缺失0、隔离0；artifact SHA=`84cf3bdc15928d703c943c060a531bf8ca68559435dea3ffa33ee7af0aa0cb7d`。
证明为`publishable=false`的隔离结果，未重跑全量5295认证，不能把上次5291与单票新结果相加
冒充新的全量PASS。原四项中XMAX取得解阻证据，TEAD70日、BGMS/CYCC190日、STEX/BSGM313日仍缺。

用户指出的全局故障扩散成立：四只均不在09-10当前PIT，却由全市场发布依赖阻断下游更新。
读取侧已解耦；发布侧仍未实现逐票降级。后续应保留全scope/分母，显式记录逐票不可用及最后
认证版本，让publication/PIT/候选和消费者校验同一可用性合同。历史整票旧版本只能用于明确
历史用途，不拼接新复权序列、不冒充当前行情；不可用证券不能产生交易信号，系统性身份/hash/
价格语义错误仍全局阻断。部分可用发布不能命名为全市场完整PASS，也不降低茶杯柄验收门槛。
这项跨消费者发布改造尚未完成，不能用一个忽略错误开关替代，也不应仅等待周一或供应商回复。

### 30.12 2026-09-13：批准的有界逐票隔离实现

30.11的发布侧改造现已在本地main实现并于SG隔离环境验证，尚未部署生产或推进正式版本。
策略BOUNDED_SECURITY_ISOLATION_V1：孤立的历史日期缺失最多占完整预期范围0.1%且不超过10只；
两个条件同时满足，不上取整。配置可收紧，不可在此版本下放宽；身份混淆、无效价格、hash/
版本错误、未知异常和必要基准缺失不适用容错。98%当日覆盖及现有坏行隔离门槛保持不变。

完整股票主范围保留，隔离股票整票从新行情分片排除，旧认证版本仅作为显式历史证据；记录
首次隔离日、缺失日期、原因、last-good版本/hash及原始响应证明。后续交易日继承名单和原分母，
不能因为无新错误就解除隔离；完整认证恢复还要求隔离期间XNYS日期及活跃股票目标日真实行情。
`--restore-isolated-security SECURITY_ID`须配合完整范围历史修复，普通增量不得隐式删除合同。
PIT资格审计保留隔离股票并注明UPSTREAM_SECURITY_ISOLATED，不进入成员；新隔离/恢复强制
重建PIT，候选绑定相同availability合同。日更编排和运维显示DEGRADED并继续正常下游，而非全市场PASS。

实际只读重新校验冻结源：完整scope8026，TEAD/BGMS/STEX共3只=0.0373785%，满足已批准额度；
不是采用受影响重认证范围5295缩小或放大分母。TEAD冻结完整源仍缺88父日期，之前另查18日
未拼接入此源，故“额外证据后仍缺70”与“正式认证缺88”须区分。BGMS缺190、STEX缺313。
校验器重新检查现有真实行及身份后产生结构化可隔离错误，没有靠旧错误字符串放行，没有
请求供应商。该探针不等于全量重新认证或正式数据发布；仍须按当时合同完成受控准备/发布、
PIT及候选验证，保留expected-parent CAS。八因子和Cursor板块轮动未改，茶杯柄发送false、
四张live表未变。详细证据及测试见[本轮实现记录](../reviews/2026-09-13-security-isolation/README.md)。
最终352项回归通过，26个已测试文件hash与本地一致，证据已归档至SG及本地review目录。

### 30.13 2026-09-13：受控发布及有界重建

30.12的隔离策略已部署SG。完整受影响范围5295只重新认证，5292只通过；TEAD88、BGMS190、
STEX313个认证日期缺失，作为3/8026=0.0373785%的逐票隔离，完整主范围及质量分母保留。
XMAX在当前完整合同下通过1934日期；不再把补齐三只作为正常股票恢复的唯一途径。
首轮cache-only发布的认证结果一致，但一次月查询展开5292文件导致首分区长期未完成，已在
正式指针不变时受控停止。writer改为每批最多25文件装载run-local DuckDB，再按月读取；源
manifest/文件hash、总行数、阶段库hash及重建价格逐列一致性都检查，CAS及原门槛不变。
真实5292文件/7868781行约41.454秒装载，首月65082行与认证源逐列完全一致；未请求供应商。
性能修复61项隔离测试和8项生产测试通过，原隔离部署另有202项生产回归通过。
实际发布时还触发软内存限额回收，临时发布任务MemoryHigh700→800→850MiB、MemoryMax900MiB保持；
正式服务定义不改。12:30:51 SGT成功发布target09-11 coverage `a5ea8408daa04e4da15735d6790154c5`，
manifest SHA `cf9054db44cabb4c0673fda61d41bc2c9825013966e010e693c0891203772992`，93分区/10514109行，
目标5713/5742=99.494949%；完整8026预期、8023实际有历史、三只无可用历史，全部发布门禁通过。
新PIT `5c6090a2d5464a429af75147cfd3d992` 于12:33:47发布，完整历史日线覆盖门禁通过，92快照、
224786成员记录、当前2846成员，精确绑定同coverage及主表5c738854ad504f1c863c47cf15bb4a63。
两者均PUBLISHED_DEGRADED而非全市场完整；availability hash一致为
`273fc3687a07bd17c38d31e65801ae8c7b331d0331de6eefb63f638bda5e045f`。
三只不入PIT/候选，276条历史资格记录false/UPSTREAM_SECURITY_ISOLATED；12:39:56保存09-14候选，
源09-11，精确覆盖2844/2846=99.929726%，v3合格1240、选入600，合同验证通过。
这恢复了正式日线→PIT→茶杯柄候选依赖；八因子/完整宽基编排未重跑，运维总任务旧FAILED及研究
BLOCKED仍保留，不能把人工分步恢复伪装成全链成功。Cursor板块和并行EP未改。发送false、v3仍0/5，
历史四表不变，分钟缺口需独立解决且不能补记失败日。版本与验收结果见
