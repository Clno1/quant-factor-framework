...
| 范围 | 状态 |
|---|---|
| GitHub 代码主线 | `main` 是唯一长期主线和默认分支；功能分支通过 PR 合入，不再使用 `master` |
...
日常开发从最新 `main` 创建短期功能分支，完成验证后通过 Pull Request 合入。SG 发布使用隔离
worktree 生成的精确白名单包，不从有未提交改动的本地工作目录直接部署。生产
