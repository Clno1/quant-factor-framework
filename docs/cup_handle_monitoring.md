...
## 30. 2026-09-11 上游修复与候选恢复条件

本轮已实际修复并部署，不再是仅巡检：FMP把HYMCZ权证、BDXA优先股存托股份错误标为普通股，
导致主表错误合并；已凭SEC证据精确纠正。12:23:47新主表target=09-10正式发布，
generation=496db448b54e4ae49701512b3acfd64c，冻结源双构建五表精确一致。
同时补齐STRR受控查询映射、ATCX/BGLC四条历史隔离证据认证，启动全证券历史恢复。
详细边界、来源证明、备份和运行路径见宽基实施30.4及SG运维48。

茶杯柄不能运行的直接原因仍是“正式coverage/PIT过期”，而不是检测CPU或内存不足。
新主表成功不等于行情成功；行情/PIT通过后才允许重建正式候选，八因子置信研究不是候选的先决条件。
截至上述恢复启动时v3仍0/5；09-09/10缺跑不能事后补记，发送保持关闭。
09-08实际运行失败的分钟缺口比例门槛是另一问题，恢复上游不代表未来完整日必然通过。
必须重新观察真实完整交易日的覆盖率、缺口、成交量证据、延迟及版本合同。

### 本轮终态：候选链已恢复，盘中验收尚待真实交易日

13:22正式coverage更新至09-10（76e68448ccea48f5b5e1dbf871c9f6c9）；13:25 PIT完整重建
通过（857031854a554d9bbfee942a6bfc3919）。421只完整历史重验均通过，六条旧坏行精确隔离，
旧失败运行不改写。新增PROP规则和冻结源重验边界见宽基实施30.4。

13:27:29 `quant-intraday-candidate-prepare.service`实际SUCCESS。今日SQLite候选快照：
- session_date=2026-09-11，source_data_date=2026-09-10；
- cup algorithm=daily-cup-5m-handle-shadow-v3，parameter_version=2026-09-01.1；
- 日线筛选2846只、满足杯体条件1206只、按配置选入600只；这不是600次盘中命中；
- data_contract绑定新coverage、PIT及496db448b54e4ae49701512b3acfd64c主表；
- candidate_snapshots外层仍沿用legacy-breakout容器键，不拿它计v3通过日；
  应读取cup_handle_daily内部算法及四张独立cup表。

此时cup_handle_cycles/evaluations/session_observations/data_gaps今日记录均为0，符合尚未开盘；
不能把盘前准备计为合格日或0%误报。CLI严格v3最近五日窗口仍为09-03/04/08/09/10，
09-08失败、其余无v3合格记录，通过0/5、剩余5日，真实env中的delivery=false。
今日18:30候选timer、21:20盘中timer均active；解除旧启动限制后等待正常开盘评估。
不需要等待八因子完成才能检测。恢复“可运行的准备链”不等于已经通过五日观察，
09-08的分钟缺口问题仍需用之后真实完整日检验，禁止补记或自动开启发送。

## 31. 2026-09-11 盘前复核与后续验收安排

16:00左右现网复核：09-11候选快照仍有效，日线筛选2846、合格1206、选入600，
v3算法和2026-09-01.1参数未变；coverage/PIT/主表绑定与30节一致。
特别注意PIT合同位于`data_contract.coverage.derived_universe`，不能仅看外层membership_sha256。
八因子已于15:17:46正式发布并验证，不再是“待计算”；它不是盘中检测启动依赖。

本次查询四张cup表的09-11记录均0，符合尚未开盘；不能提前计为运行成功。
当前v3通过日期为空、0/5、剩余5个完整合格日；09-08分钟缺口失败与09-09/10缺跑均保留。
真实env再次确认`cup_handle.delivery_enabled=false`。不借用v2的2个通过日或宽基数据shadow的1/5。

下一批检查由原Codex任务sg覆盖：18:45核查18:30候选任务；21:45核对21:20服务启动后
是否实际出现五分钟评估；次日04:45（当前夏令时收盘后）核查完整日结；11:45核对共享上游。
按XNYS实际开闭市判断，休市不计数。任务已保存到Codex，SG原生产timer未更改。
每次核对持续落库、85%周期覆盖率、5%错误周期上限、95%可评估覆盖率、5%缺口证券上限、
250ms P95、96根长度以及完整版本合同。非正成交量不作有效证据，零信号不等于0%误报。
无变化保持安静，有实质新结果或需决策时通知；5/5后只提交人工验收，不自动发Discord。

本次同时修复专项页早期主表失败覆盖较晚正式成功的显示错误，未抹除茶杯柄09-08失败。
运维站16:05:23已验证该失败继续显示；备份、测试和部署证据见SG运维49、可观测性46。

### 18:53至18:56 SGT：盘前定时任务复核

`quant-intraday-candidate-prepare.service`已由timer在18:30:42启动，18:30:45成功退出，
CPU时间2.587秒，阶段candidates_prepared。复用13:27:29生成且仍匹配正式版本的快照：
v3/2026-09-01.1，筛选2846、杯体合格1206、候选600，source=09-10；不是重新生成盘中信号。
coverage/PIT/主表/八因子版本与本节此前验收一致，没有重复启动全量重算。

四张cup表09-11记录仍0，21:20盘中timer仍enabled/active，符合尚未开盘。
v3通过日期为空、0/5、剩余5日，发送false；09-08的3命中/2183拒绝/483等待/91不可评估/
0错误及13个唯一缺口均是旧证据，不冒充今日评估。ABG四条零突破成交量仍REJECTED、signal=null，
未生成有效成交量比例。MDB回放仍为v1、零信号、误报代理null，不能证明v3或0%误报。
资源检查PASS：可用内存1123.34MB、磁盘29.177GiB。候选本次MemoryPeak未保留，不能填成0。
watchdog正常结束，运维healthz 200；16:05后相关服务无error级journal记录。
运维站仍保留09-08失败与v3的0/5，未被SCHEDULED覆盖。无新阻断，继续等待真实盘中，
未重启服务、修改门槛、发送或补记通过。

## 32. 2026-09-11 开盘后首次真实运行确认

21:45巡检于21:49开始读取实际证据，并于22:08复核。盘中service在21:20:12由timer启动，
active/running、NRestarts=0；heartbeat的session=09-11、market_open=true，source=09-10，
cup算法v3、参数2026-09-01.1。候选600只（筛选2846、杯体合格1206），绑定现网正式
coverage/PIT/主表，八因子代次未变；不是仅凭active判断恢复。

21:49首次读取为4批/160次评估，之后截至22:05:08已持续增长至7批/280次、48只唯一股票。
各批本地时间21:30、21:36、21:41、21:47、21:53、21:59、22:05，均为40只，
data_contract_complete均1，算法/参数一致。候选600不等于600只同时进入每批活动池。

- 截至22:05：命中0、拒绝0、等待277、不可评估3、cup检测错误0；最大7根。
- 前八原因中实际仅四类：INSUFFICIENT_COMPLETED_5M_BARS=226、NO_COMPLETED_5M_BARS=36、
  STALE_QUOTE=15、UNRESOLVED_5M_SOURCE_GAP=3。
- 280条实际latency复算P95=0.015572ms；大多数处于前置等待，不能推断完整柄检测负载。
- 当前唯一缺口2个、涉及WBI一只：UNRESOLVED_SOURCE_GAP=2，另外两种确认分类均0。
  涉及美东09:35至09:40、09:40至09:45桶；报价供应商时间仍为09:30:01，
  累计成交量差550/582不能定位到对应桶，证据理由为VOLUME_DELTA_NOT_LOCALIZED_TO_BUCKET。
  这是数据证据不足，不能判成无成交、已确认供应商缺口或用平价OHLCV补齐。
- 按当前唯一样本口径，暂时可评估47/48=97.92%、缺口股1/48=2.08%；尚非收盘验收。
  cup_handle_session_observations当日0行，7批不能据此提前宣告85%全日周期门槛通过。
- 当前INSUFFICIENT_VOLUME_EVIDENCE=0；09-08 ABG四次零突破成交量仍REJECTED且signal=null。
  MDB离线回放仍v1、110根、零信号/误报代理null，不充当v3误报率证明。

共享monitor_cycles另有21:36、21:41两次error_count=1，其粒度不是cup检测错误；现有行不含
逐票失败细节，不推定为算法错误或编造其具体原因。cup错误仍0，journal无error级条目，服务未重启。
22:08盘中服务当前/峰值内存437.61/438.11MiB，swap峰值0，累计CPU28.07秒；资源检查PASS，
可用内存999.418MB、磁盘28.826GiB。watchdog正常、运维healthz 200且快照新鲜。

运维快照已显示当前RUNNING并保留09-08失败原因和完整日记录；其可评估覆盖率/13个缺口等部分
质量指标仍来自最近完整日，不能当成今日值。今日值以上述SQLite分session/algorithm统计为准。
v3通过日期仍为空、0/5、剩余5日，cup delivery=false，未启动额外任务或改规则。
本轮仅通知“首次真实运行恢复及WBI缺口风险”；明日04:45按真实收盘结果验收，不补记缺跑日。

## 33. 2026-09-11 22:48至22:52 SGT：v3独立验收复核

本轮通过SSH运行正式`--status`，读取四张cup表的一致只读事务快照，并检查候选、service、
timer、journal和运维API。以下今日统计固定截至22:50批次，不与22:48的14批/560次混合。
当时美东10:50，尚未收盘，不调用finalize，不写入通过记录。

### 最近五个完整交易日

XNYS实际窗口为09-03、09-04、09-08、09-09、09-10；v3通过日期为空，0/5，仍缺五个完整合格日。
09-03只有v2候选/观察，不计v3；09-04/09/10无候选、v3周期或评估，不把缺跑当成零错误通过。
09-08为v3 FAIL：日线筛选2846、杯体合格1300、选入600，实际56只/2760次；
命中3、拒绝2183、等待483、不可评估91、错误0。70/78周期=89.74%，错误周期0%，
P95=0.563372ms、最大76根、所有70批合同完整、参数均2026-09-01.1。
失败仍为53/56=94.64%可评估覆盖低于95%、3/56=5.36%缺口股高于5%。
唯一缺口13，NO_TRADE_CONFIRMED=1、PROVIDER_GAP_CONFIRMED=0、UNRESOLVED_SOURCE_GAP=12。
前八原因依次为HANDLE_TOO_SHALLOW=1843、INSUFFICIENT_COMPLETED_5M_BARS=311、
HANDLE_VOLUME_NOT_CONTRACTING=170、RIM_NOT_BROKEN=166、STALE_QUOTE=104、
UNRESOLVED_5M_SOURCE_GAP=91、HANDLE_NOT_FORMED=67、INSUFFICIENT_VOLUME_EVIDENCE=4。
...
| 项目 | 截至22:50的真实值 |
| --- | --- |
| 日线筛选 / 杯体合格 / 选入候选 | 2846 / 1206 / 600 |
| v3周期 / 评估次数 / 唯一股票 | 15 / 600 / 50 |
| 命中 / 拒绝 / 等待 / 不可评估 / cup错误 | 0 / 224 / 362 / 14 / 0 |
| 可评估覆盖 / 缺口股比例 | 48/50=96% / 2/50=4% |
| 检测P95 / 最大bar_count | 0.245137ms / 16 |
| cup错误周期比例 / 合同 / 参数 | 0/15 / 15批全部完整 / 2026-09-01.1 |
| 当日完整观察 | 尚未生成，不能按15/78提前宣布全日通过或失败 |

真实env门槛：5分钟、周期覆盖率至少85%、错误周期不超过5%、可评估至少95%、缺口股不超过5%、
P95不超过250ms、最多96根、5个完整合格交易日。cup delivery=false，5/5后也只允许人工验收。
今日前八拒绝/等待/不可评估原因如下，不能把正常形态拒绝等同程序错误：

| 原因 | 次数 |
| --- | ---: |
| INSUFFICIENT_COMPLETED_5M_BARS | 302 |
| HANDLE_TOO_SHALLOW | 177 |
| NO_COMPLETED_5M_BARS | 36 |
| RIM_NOT_BROKEN | 29 |
| STALE_QUOTE | 24 |
| HANDLE_VOLUME_NOT_CONTRACTING | 17 |
| UNRESOLVED_5M_SOURCE_GAP | 14 |
| INSUFFICIENT_VOLUME_EVIDENCE | 1 |

### 缺口与成交量证据

今日唯一缺口8个，不将重复观测累加成新事件：WBI=2、UAN=6；分类全部为
UNRESOLVED_SOURCE_GAP，NO_TRADE_CONFIRMED=0、PROVIDER_GAP_CONFIRMED=0。
22:48时还只有3个，22:50 UAN刷新又识别出5个历史桶，故以同一SQLite快照的8个为准。

- WBI美东09:35、09:40起始的两个桶，供应商报价时间停在09:30:01，累计量差550/582
  不能定位到桶内。UAN09:50桶同类问题，累计量差43、供应商时间09:46:08。
- UAN10:15、10:20、10:30、10:35、10:40起始的五个桶，累计量差0，但缺少新鲜度水位，
  原因NO_FRESHNESS_WATERMARK。0增量不等于已证明该桶无成交。
- 两只股票已正确进入不可评估。建议保留原始分钟/报价时间戳向供应商核对，只有来源证据完整
  才能重分类；不凭零增量补平价OHLCV，不删除问题股票绕过门槛，也不补记既往失败日。

今日IBTA在22:50因breakout_volume=0进入INSUFFICIENT_VOLUME_EVIDENCE，signal=null、
ratios={}；baseline=2173、handle=2887.65407。09-08 ABG四次同类保护依旧成立。
本次扫描窗口内及今日v3 payload，未发现非正baseline/handle/breakout成交量与有效比例并存。
SG detector代码SHA-256与本地一致，非正检查位于除法前，未修改算法。

### 误报代理与运行安全

MDB保存的回放仍为v1、2026-08-10至11、110根、0信号、false_positive_rate_proxy=null。
今天v3无信号，因此误报代理尚不可估计，不是0%。09-08有3次MATCH但仅2条去重v3信号
（VTS、NKTR），其保存payload无后验代理；当前发现的回放报告亦无v3后验结果，不能报告误报率。
配置代理是未来6根完整五分钟K线内先达+2%还是先破柄低；完整后续数据和版本合同齐备后
可独立生成只读后验报告，不用旧v1结果或缺失观察冒充现网v3表现。

候选18:30:45成功，盘中21:20:12启动且NRestarts=0；watchdog正常，operations Web长驻运行，
不存在operations-web.timer属正常架构。候选timer下次09-14 18:30，盘中timer已触发且service持续运行。
22:49盘中当前/峰值内存约377.75/440.00MiB、swap0、累计CPU51.52秒；watchdog峰值约111.70MiB，
operations Web当前约40.98MiB。资源PASS：可用内存1000.895MB、磁盘28.738GiB。
相关journal无error级条目；共享monitor_cycles仍有21:36/21:41两次error_count=1，不能与
cup的0次检测错误混同；历史逐票原因未保存在这些行内，本轮不推测具体网络或股票故障。

运维API保持RUNNING、v3、0/5及09-08完整日FAIL；94.64%/13个缺口等字段仍取最近完整日，
不能误读成今日96%/8个缺口。后续展示优化应显式分隔“今日盘中”和“最近完整日”，保留失败与版本切换。
正式主表/coverage/PIT/八因子版本与32节完全一致，无当前上游过期阻断，未重复重算或重启服务。
本轮仅更新三份运维文档；共享上游未变，不改写宽基实施文档，不修改生产规则、发送开关或历史台账。

## 34. 2026-09-12 10:39至10:44 SGT：09-11完整交易日FAIL

本节用真实日结替代33节的盘中进度。SSH正式`--status`返回0，SQLite只读事务独立复算
四张cup表，与04:05:09 SGT生成的09-11日结在评估数、周期数、P95和覆盖率上完全一致。
只按daily-cup-5m-handle-shadow-v3统计，未调用finalize或改写日结。

### 五日窗口与结论

最近五个完整XNYS交易日为09-04、09-08、09-09、09-10、09-11。
通过日期为空，v3仍0/5，剩余五个完整合格交易日；09-08和09-11均FAIL，09-04/09/10缺跑。
09-03的v2已移出窗口且从未计入v3；09-11旧动量390/390的PASS也不能替代cup独立验收。
09-12周六不计观察日。真实env仍cup_handle.delivery_enabled=false，5/5以后也仅提交人工验收。

| 09-11项目 | 独立复算结果 | 门槛与结论 |
| --- | --- | --- |
| 日线筛选 / 杯体合格 / 选入 | 2846 / 1206 / 600 | source=09-10，正式coverage/PIT绑定有效 |
| 周期 / 实际评估 / 唯一股票 | 70 / 2800 / 56 | 每批40只活动股票，非600只同时评估 |
| 周期覆盖率 | 70/78=89.7436% | 至少85%，通过 |
| cup错误次数 / 错误周期比例 | 0 / 0% | 不超过5%，通过 |
| 命中 / 拒绝 / 等待 / 不可评估 | 0 / 2243 / 480 / 77 | 不是零信号即通过 |
| 可评估股票覆盖率 | 53/56=94.6429% | 低于95%，失败 |
| 缺口股票比例 | 3/56=5.3571% | 高于5%，失败 |
| 检测P95 / 最大bar_count | 0.588805ms / 77 | 不超过250ms / 96，通过 |
| 算法、参数和合同 | v3 / 2026-09-01.1 / 70批合同均完整 | 与日线候选及正式版本一致 |

首个cup周期为美东09:30，最后15:59；服务于美东16:05:09正常完成，并非缺跑、重启或OOM。
日结失败原因明确为INSUFFICIENT_EVALUABLE_TICKER_COVERAGE、EXCESSIVE_MINUTE_DATA_GAPS。

### 唯一缺口及根因

12个唯一事件涉及UAN=9、WBI=2、IBTA=1，不将重复observation_count或77次不可评估计为新事件。
NO_TRADE_CONFIRMED=0、PROVIDER_GAP_CONFIRMED=0、UNRESOLVED_SOURCE_GAP=12。

- UAN美东09:50桶累计量差43，但供应商时间09:46:08，不能将成交定位到09:50至09:55。
  其10:15、10:20、10:30、10:35、10:40、10:55、11:00、11:10起始的8个桶为
  NO_FRESHNESS_WATERMARK：累计量差0但没有新鲜度证明，不能认定无成交。
- WBI09:35、09:40起始的两个桶累计量差550/582，供应商时间停在09:30:01，仍无法定位成交。
- IBTA15:05至15:10桶累计量差672；边界后报价时间15:10:07，不能把跨边界的累计量差全部
  归入该桶，首次记录于美东15:16。它成为第三只缺口股票，使完整日最终超过5%上限。
- 总计4个VOLUME_DELTA_NOT_LOCALIZED_TO_BUCKET、8个NO_FRESHNESS_WATERMARK。
  当前证据只支持“分钟数据证据不足”，不支持把12个事件改成无成交或已确认供应商漏数。

修复建议：先逐桶核对上述三只股票的原始五分钟行情、累计量和供应商时间戳，取得可信的
成交归属/无成交/新鲜度证明；若现有源无法提供，需人工评估补充分钟数据源。
不能用扩大缺口容忍度、排除问题股、平价OHLCV补齐或重跑八因子绕过问题。
本次不更换供应商、不重分类原始事件、不补记09-08/11失败日，也不重启已正常完成的服务。

### 前八拒绝或等待原因

| 原因 | 次数 |
| --- | ---: |
| HANDLE_TOO_SHALLOW | 1793 |
| INSUFFICIENT_COMPLETED_5M_BARS | 302 |
| HANDLE_VOLUME_NOT_CONTRACTING | 289 |
| RIM_NOT_BROKEN | 156 |
| STALE_QUOTE | 92 |
| UNRESOLVED_5M_SOURCE_GAP | 77 |
| HANDLE_NOT_FORMED | 49 |
| NO_COMPLETED_5M_BARS | 36 |

其余为HANDLE_EXCEEDS_CUP_DEPTH_RATIO=3、INSUFFICIENT_VOLUME_EVIDENCE=2、STALE_COMPLETED_5M_BAR=1。
两个INSUFFICIENT_VOLUME_EVIDENCE均为IBTA的breakout_volume=0，signal=null、ratios={}；
09-08 ABG四次同类保护仍成立。扫描现有全部v3评估payload，无非正成交量与有效比例并存。
SG detector及现有测试文件与本地SHA-256一致。本地`tests/test_cup_handle.py`共12项通过；
另使用现有有效形态，按实际检测出的handle_start划定基线/柄区间，分别测试baseline、handle、
breakout为0和-1共6种情况，全部拒绝且没有signal/ratio。临时验证不改生产数据或测试文件。

### 误报代理与服务状态

MDB保存的回放仍是v1、08-10至11、110根、0信号、false_positive_rate_proxy=null；
不是v3验证，也不是0%误报。09-11实际无信号，误报率不可估计。09-08保存的两条唯一v3信号
VTS/NKTR仍为SHADOW，未发现带版本合同的v3后验代理报告，不能据此报告命中率或误报率。
需具备完整的未来6根五分钟K线后再生成独立后验报告；不以缺失观察或v1回放补齐。

盘中service从09-11 21:20:12至09-12 04:05:09正常运行、exit=0、NRestarts=0；
峰值内存440.0MiB、swap0、CPU381.173秒。候选18:30:45成功；watchdog10:39:05成功，
operations Web正常且healthz 200、快照约27秒新鲜。该Web昨晚23:43:37至38完成有序重启，
journal无warning/error，未中断盘中服务；本轮未发起重启。
资源PASS：可用内存1126.281MB、磁盘27.923GiB；watchdog峰值约111.62MiB，Web当前约40.28MiB。
盘中/候选timer下次为09-14 21:20/18:30，休市不强行启动；operations Web长驻，无独立timer。
旧动量390/390、两次错误周期、日结PASS只属于共享monitor，cup独立错误仍0、日结仍FAIL。

10:41:20运维快照已将最近完整日更新为09-11、状态DEGRADED、阶段completed_session，保留
上述两个失败原因和v3的0/5，没有被周末SCHEDULED或旧动量PASS覆盖。
宽基发布版本仍为09-10原版本，但新目标推进至09-11，专项前四阶段显示STALE；今天11:30:19
常态日更尚未执行，不能把待更新状态说成昨晚候选阻断或新任务失败。详见宽基实施30.6。
本轮更新四份文档，未修改生产规则、发送开关、原始行情或历史台账。

## 35. 2026-09-12 下午：区分已运行、分钟验收失败与新上游阻断

本轮SSH正式status及17:02 SGT四张cup表只读复查，34节完整日统计未变：v3最近五个完整XNYS日
09-04/08/09/10/11，通过日期为空、0/5；08和11失败，其余缺跑。09-11有正式候选600、
70/78周期、2800次评估，命中0/拒绝2243/等待480/不可评估77/错误0；P95=0.588805ms、
最大77根。可评估53/56=94.6429%及缺口股3/56=5.3571%仍不合格，70批版本合同完整。
不是shadow未开启，而是已运行的shadow没有通过发送前验收。前八原因及完整合同见34节。

### 分钟源复查与不可绕过的证据缺口

16:47 SGT使用现有`diagnose_cup_handle_data_gaps.py`对09-11原12个唯一缺口重新查询：

| 股票 | 原唯一缺口 | 事后1min/5min查询结果 |
| --- | ---: | --- |
| UAN | 9 | 九个桶两种间隔均未返回记录 |
| IBTA | 1 | 15:05桶两种间隔均未返回记录 |
| WBI | 2 | 09:35为1/1条，09:40为0/1条；native 5min与1min聚合不一致 |

原始诊断在SG的`outputs/data_audits/cup_handle_gaps/2026-09-11_f45490dccd8e4e6cb635b289a1a8236e.json`。
本地冻结证据在`reviews/2026-09-12-cup-shadow-investigation/`，含完整诊断、逐桶对比及未发送的
供应商核对问题。以上是查询时刻的响应，不代表盘中已经存在；历史日结和缺口表均未改写。
当日分类仍为NO_TRADE_CONFIRMED=0、PROVIDER_GAP_CONFIRMED=0、UNRESOLVED_SOURCE_GAP=12。

WBI09:35的1min记录volume=120，native 5min=453；09:40 native 5min=497而1min缺失。
整日共同桶中IBTA/UAN/WBI的成交量分别有53/17/34桶不一致，价格也存在差异。
现网使用1min聚合；不能未经时间戳、成交条件、成交量及修订语义认证就混入native 5min。
UAN/IBTA仍缺少可定位成交或证明无成交的新鲜水位，空响应不等于无成交。
建议供应商逐桶说明最终性、时间标签和缺失原因；必要时评估第二真实数据源并独立验证数据合同。
本轮没有更换源、排除股票、补造OHLCV、重分类旧事件或补记失败日。

### 成交量保护与误报证据

新增持久回归测试覆盖基线、柄、突破三个区域各为0/-1，共六种非正成交量场景。
均须INSUFFICIENT_VOLUME_EVIDENCE、无signal且无handle/breakout比例；先验证正成交量对照确实MATCH。
SG隔离测试文件与现有gap/master测试合跑33项通过，部署配置后复跑仍33项通过（1.76秒）；本轮本地无可用项目venv，
不将SG测试表述为本地执行。实际IBTA两条零突破成交量拒绝仍正确。
MDB保存回放仍是v1、110根、零信号、代理null，不算v3验证；09-08的VTS/NKTR仍无合格后验报告。
09-11零信号只能说误报率不可估计，不能报告0%误报。

### 新上游故障与恢复边界

今天11:30初跑及12:06重试，正式主表均因退市历史100页不够覆盖2019-01-01而失败；
12:41又触发StartLimit。这是下个交易日候选的上游风险，不是09-11分钟缺口的原因。
本轮仅将`data.security_master.delisted_max_pages`从100增至200并部署SG，保留所有质量门槛。
16:56受锁和内存限制的恢复任务启动，17:02主表已用102页覆盖历史边界并PASS；
日线行情/PIT及下个交易日准备性的最终结果见宽基实施30.7，不能仅凭主表成功就宣称整链恢复。

17:01:44运维API仍为DEGRADED/completed_session，即使target_session推进09-14，仍保留
09-11 FAIL、v3及0/5，没有被SCHEDULED覆盖。healthz健康、快照56秒内；候选/盘中timer
下次09-14 18:30/21:20，Web长驻无timer。盘中已正常结束、无重启、峰值440MiB、swap0。
cup delivery仍false；至少还需五个未来连续完整合格交易日，达到5/5也只提交人工验收。

### 本轮终态与合同门槛修复

17:30:56上游恢复整链FAILED：主表分页已修好，但coverage有5295只非统一价格/量修订，
PIT未执行。日线正式版本仍09-10；17:32对09-14/source=09-11的隔离候选检查明确stale并阻断，
没有写入候选表或四张cup表。5295只中4680只在匹配范围内仅末日修订、615只还有更早差异，
另12行未匹配；必须分流认证，不能盲目全历史重建或放宽容差。详见宽基30.7终态。
因此当前既有历史分钟质量FAIL，也有下个交易日的日线准备阻断，不能宣布“等待五天即可开启”。

另发现合同日结原来以any聚合：一批完整可能掩盖另一批不完整。新增测试先在未改SG代码上
复现误放行，再将cup专用日结收紧为`bool(cycles) and all(...)`，旧动量方法未改。
四种完整/不完整批次组合、六种非正成交量场景及相关回归共62项通过；17:30:49部署SG后
使用真实部署模块复跑仍62项通过（5.79秒）。测试均使用临时数据库，不重算生产历史日结。
部署备份`outputs/deploy_backups/cup_shadow_20260912_pagination/state.py.before`，新源码SHA-256=
`e922d72395353b9118866312c9c32ec79e890ed3a859566cf841219e4ebd960b`。
这修复潜在漏检，不是09-11失败原因：该日已独立确认全部70批合同完整。
算法仍v3、参数仍2026-09-01.1，发送仍false，0/5不变。

还需注意，现网日结按“全天曾有缺口的股票”计入缺口股集合，不因后续补齐或改为确认分类
自动移出。因此供应商确认无成交也不自动让v3通过，更不能生成零量平价K线绕过门槛；
任何未来数据语义/可评估规则变化都应单独审查版本合同，旧失败日不能重分类为通过。

17:42最终正式status仍v3 0/5、effective_cup_handle_mode=shadow、cup delivery=false；
运维API仍保留09-11 FAIL，healthz正常且快照36秒新鲜。恢复进程已结束，临时/run时限配置已清理。
主机可用内存1034MiB、磁盘28GiB、swap0；没有将未完成的数据修复说成成功。

## 36. 2026-09-12 18:13：源修订认证试点，不是发送放行

下一步已实现并部署审计工具`scripts/audit_coverage_revisions.py`，没有发布参数，不改变正式
coverage/PIT指针、shadow日结或发送开关。按正式历史身份映射、16个冻结XNYS日期和421份已认证
canonical缓存重建5295只失败范围，得到4844只末日修订候选、450只更早历史变化、1只日期覆盖
阻断（CEPS）。这是认证输入重建口径，替代35节当前ticker连接的初步诊断，不混用两组计数。

BRBS、UNCY、NKE全历史真实源回查分别核验1932、1297、1932行保留前缀，全部逐字段完全一致，
09-10修订和09-11新行也与冻结源一致；三只可标记VERIFIED_LOCAL_REVISION，但publishable仍false。
其余4841只候选未经完整前缀认证；450只和CEPS不能借用样本通过。正式行情仍09-10，尚未恢复
下周一候选的上游准备。报告与版本合同见宽基30.8，SG部署模块71项回归通过。

18:11正式status及四张cup表复核：通过日期仍为空，最近五个完整XNYS日09-04/08/09/10/11仍0/5。
09-11的600候选、70/78周期、2800评估、命中0/拒绝2243/等待480/不可评估77/错误0未变；
P95=0.588805ms、最大77根、70批合同完整。可评估53/56=94.6429%、缺口3/56=5.3571%，仍FAIL。
前八原因仍为HANDLE_TOO_SHALLOW1793、INSUFFICIENT_COMPLETED_5M_BARS302、
HANDLE_VOLUME_NOT_CONTRACTING289、RIM_NOT_BROKEN156、STALE_QUOTE92、
UNRESOLVED_5M_SOURCE_GAP77、HANDLE_NOT_FORMED49、NO_COMPLETED_5M_BARS36。

唯一缺口仍IBTA1/UAN9/WBI2，共12个UNRESOLVED_SOURCE_GAP，NO_TRADE_CONFIRMED和
PROVIDER_GAP_CONFIRMED均0；本轮没有新分钟源证据或重分类。IBTA两次breakout_volume=0
均INSUFFICIENT_VOLUME_EVIDENCE、signal=null且无有效成交量比例。MDB旧v1零信号回放不能
替代v3，零信号也不是0%误报；未用本轮日线源修订样本补齐v3后验代理。

18:13运维站认证后healthz正常、快照68秒，target虽为09-14，仍DEGRADED/completed_session并
保留09-11 FAIL和v3 0/5。cup delivery=false，仍至少需要五个未来连续完整合格交易日，且须先
解除上游阻断；没有承诺五个自然日后开启，达到5/5也只供人工验收。源证据及完整服务资源在
`reviews/2026-09-12-coverage-revision-authentication/`，没有修改其他并行EP工作。

## 37. 2026-09-12 20:15：全量认证进行中，分钟与后验独立核验

本轮在SG实现有界整票canonical认证：25只一批读取父历史、最多2个provider worker，支持
只准备不发布和完整缓存只读发布。50只试点全部通过且未发布；19:46启动完整5295只认证，
20:14检查点完成740只、738只通过、TEAD/SNYR两只失败，其余仍在运行。详细合同和最终结果
以`reviews/2026-09-12-coverage-controlled-recovery/`及宽基30.9为准，不能把进度当成整链成功。

TEAD原始OB查询缺88个已认证日期，换TEAD查询也只返回其中18日，尚缺70日；SNYR触发未获
审核的无效行情隔离检查。未混入旧前缀、扩大隔离名单、删除问题股或放宽容差。20:11实际
PIT前置检查仍因Coverage/当前主表版本不同阻断；09-14候选因正式行情仍09-10而stale。
没有发布未来候选或写入观察日，不能只等待五天。

分钟源再次查询后，09-11仍UAN9/IBTA1/WBI2共12个唯一未决事件；NO_TRADE_CONFIRMED=0、
PROVIDER_GAP_CONFIRMED=0、UNRESOLVED_SOURCE_GAP=12。WBI后来的5min响应不能证明盘中
1min完整，也不能未经合同核验混源。相关环境仅发现FMP凭据，已询问是否有独立行情订阅，
未索要密钥。旧分类和日结不重写，缺口问题与日线上游分开处理。

新增只读`scripts/audit_cup_handle_followthrough.py`绑定09-08的两个真实v3信号及原始候选，
保存原始1min响应；必须连续30个有效正成交量分钟，并排除触发分钟和同分钟屏障先后歧义。
NKTR完整窗口11:29 ET先触柄底76.2，+2%目标79.3101未先到，记STOP_FIRST失败代理；
VTS10:48 OHLC18.115、volume=0，记UNRESOLVED/NONPOSITIVE_OHLCV。可判定样本1/1失败，
另1/2未决；全样本代理百分比为null，不是0%，小样本也不代表稳定误报率或执行盈亏。
17项新边界测试在SG实际部署模块通过。报告SHA及原始响应见本轮review。

MDB现算法回放已尝试，但被PIT/主表代际不一致阻断；没有把旧v1零信号报告当v3结果。
09-11零信号仍只能说误报率不可估计。20:11正式status仍为v3/2026-09-01.1、shadow、发送关闭；
最近五个完整XNYS日09-04/08/09/10/11通过日期为空、0/5。09-11候选600、评估2800、
命中0/拒绝2243/等待480/不可评估77/错误0，周期70/78，可评估53/56、缺口3/56，
P95=0.588805ms、最大77根、70批合同完整；前八原因与36节相同。仍至少需五个未来连续完整
合格交易日，且要先解决现存阻断；5/5也只交人工验收，不自动发送。

## 38. 2026-09-12 23:36：全量复验完成，31项修复，四项上游仍阻断

SG原策略完整检查5295只，5260通过、35失败；审核修复策略受控部署后，用更严格父身份校验
重新检查全部5295只，5291通过、4失败，原失败中31项实际修复。15只按已核验发行人/别名窗口
重新抓取整票历史，16只继承逐字段完全一致的既有隔离；没有拼旧前缀、删除有效日期或补行情。
原报告和复验报告分别保留，终态、哈希、逐票证明见宽基30.10及本轮review。

剩余阻断：TEAD原OB缺88日，后续代码只能找到18日，仍缺70日；BGMS/CYCC缺190日、
STEX/BSGM缺313日，后续代码的三个FMP端点全部为空。XMAX两日真实数据存在于XWIN，但
尚缺正式换代码日期的充分独立证据，不批准映射。不能以5291只通过替代完整范围发布。
coverage正式发布未执行，PIT仍因主表代际不一致阻断，09-14候选仍因正式日线09-10而stale。
23:36真实MDB回放也报`[US_LIQUID_5M] PIT and Security Master generations differ`；
MDB日线1934行认证通过不等于回放通过，旧v1报告没有计入。

23:36再次运行指定`--status`并读四张cup表：算法仅`daily-cup-5m-handle-shadow-v3`、
参数`2026-09-01.1`；最近五个完整连续XNYS交易日为09-04/08/09/10/11，通过日期为空、0/5。
09-08和09-11失败，09-04/09/10缺失；不借用旧版本、旧动量PASS、休市日或未运行日。
四表v3行数140/5560/2/25及全部内容哈希与20:15一致，未补记观察或更改历史失败。

最近完整日09-11：筛选2846、合格1206、候选600；实际70/78五分钟周期=89.7436%，
2800次评估，命中0、拒绝2243、等待480、不可评估77、错误0；涉及56只，可评估53只=
94.6429%低于95%，缺口3只=5.3571%高于5%，因此FAIL。周期门槛85%、错误率上限5%均未改；
P95=0.588805ms低于250ms，最大bar_count=77低于96，70批算法/参数/候选/数据合同完整。
前八拒绝/等待原因：HANDLE_TOO_SHALLOW1793、INSUFFICIENT_COMPLETED_5M_BARS302、
HANDLE_VOLUME_NOT_CONTRACTING289、RIM_NOT_BROKEN156、STALE_QUOTE92、
UNRESOLVED_5M_SOURCE_GAP77、HANDLE_NOT_FORMED49、NO_COMPLETED_5M_BARS36。

唯一缺口UAN9/IBTA1/WBI2，共12；NO_TRADE_CONFIRMED=0、PROVIDER_GAP_CONFIRMED=0、
UNRESOLVED_SOURCE_GAP=12。再次取证仍无独立完整分钟源，WBI原生5分钟与1分钟结果不一致，
没有混源或合成零成交量bar。IBTA两次INSUFFICIENT_VOLUME_EVIDENCE均无信号和有效比例；
非正baseline/handle/breakout量的回归仍拒绝。09-08 NKTR为STOP_FIRST失败代理，VTS零量未决；
可判定子样本1/1失败，全样本比例null；09-11零信号也不是0%误报。后验不回写验收。

服务资源见SG55；运维API仍DEGRADED/completed_session，target虽09-14仍保留09-11 FAIL。
`intraday_momentum_monitor.cup_handle.delivery_enabled=false`未变，仍至少需要解除阻断后
五个未来连续完整合格交易日，不能承诺五个自然日后开启；5/5也只报告人工验收。
135项数据链回归和41项运维/cup/后验回归通过。原始证据、未发送的供应商修复请求草稿在
`reviews/2026-09-12-coverage-controlled-recovery/`，没有购买数据或发送外部工单。

## 39. 2026-09-13 00:09-00:10：对照原始要求的未完成清单

本轮通过SSH重新运行指定status，只读读取四张v3表，检查四服务、timer/journal和认证运维API，
并实际重试不发布的PIT、09-14候选验证和MDB回放。没有修改算法、发送、行情或历史日结。

| 原始验收要求中仍未满足的项 | 当前证据 | 完成条件 |
| --- | --- | --- |
| 下个交易日正式coverage/PIT绑定的候选 | coverage仍09-10；PIT报当前主表代际不一致，09-14候选报日线stale | 解决38节四只股票的源/身份证据，完整认证后受控发布coverage、绑定PIT并验证正式候选 |
| 可评估股票覆盖率>=95%、缺口股票比例<=5% | 09-11仍53/56=94.6429%、3/56=5.3571%，唯一未决缺口12个 | 独立修复分钟数据问题，使未来实际完整交易日达到原门槛；不把补查证据变成历史PASS |
| MDB现网v3回放及后验检查 | MDB本轮仍因PIT与主表代际不同exit1；既有后验NKTR失败、VTS因零成交量未决 | 上游恢复后完成v3回放；补充可核验的后验数据，无法判定则继续明确报告未决 |
| 五个连续完整合格XNYS交易日 | 09-04/08/09/10/11仍0/5；08/11失败，其余三日缺失 | 取得五个连续完整合格日；只提交人工验收，不自动开启发送 |

以上是原要求的剩余项，不是新增四套任务。第一项和MDB共享同一个上游阻断；五天是实际运行
验收，不是修复代码前先等待。第二数据源仅为核验手段之一，不是必须采购的新前置条件。
全量5295只中31项原失败已修复、剩余4项未解决的证据仍见38节和宽基30.10，未重跑无变化的全量任务。

已完成的检查不重复列为待开发：09-11实际70/78周期、2800次评估，命中0/拒绝2243/等待480/
不可评估77/错误0；P95=0.588805ms、最大77根。四表v3行数仍140/5560/2/25；参数仍2026-09-01.1。
候选600、70批完整合同、前八原因、非正成交量保护及后验原始证明沿用38节已核验证据，
本轮未重新抓分钟源或重跑测试。唯一缺口分类仍NO_TRADE_CONFIRMED=0、PROVIDER_GAP_CONFIRMED=0、
UNRESOLVED_SOURCE_GAP=12（09-11口径，不混入09-08事件）。零信号仍不是0%误报。

运维API仍DEGRADED/completed_session并保留09-11 FAIL与v3；healthz=ok、快照42秒。
发送仍false，通过日期为空，仍缺五个未来连续完整合格日。共享宽基正式版本未变，
本轮不改宽基30.10已记录的终态；三份运维文档只追加本次检查结论。

## 40. 2026-09-13 03:18-03:29：本地修复、SG隔离验证与真实剩余阻断

本轮在本地main开发，在SG独立临时项目运行测试及真实只读验证，**未提交、push或部署本轮
补丁到生产**。上一轮已部署代码不等于本轮新代码已上线；未动Cursor板块轮动分支或并行EP。
完整机制、源证据和发布侧容错边界见
[本轮审计](../reviews/2026-09-13-cup-resilience/README.md)。297项回归通过，含65项共享/隔离测试。

修正此前过宽的“非正成交量保护已完成”表述：旧代码只验证平均量，单根零量可藏在正平均中。
本轮复现后，逐根校验实际baseline/handle/breakout窗口，并把无效原始分钟量作为证据保留到
五分钟聚合；零、负、NaN、Inf均不得形成有效比例或信号，返回INSUFFICIENT_VOLUME_EVIDENCE。
另修复稀疏分钟缓存跨五分钟完成边界不刷新的问题，仍保留真实source_minute_count，不补bar。

读取侧改为校验并加载coverage/PIT精确绑定的已发布主表，而非强行读取最新主表；ID、hash、
完整性和日期新鲜度仍检查。真实MDB v3回放因此恢复：2个session、累计110次评估，单次最大
bar_count=78、P95=0.574459ms、信号0、误报代理null。110是累计次数，不是超96根序列。
回放后验复用严格逐分钟核验：缺失、不连续、重复、非正量或无法确定触发后先后顺序均未决；
存在未决或零信号时全样本误报率为null。既有NKTR STOP_FIRST/VTS零量未决证据未改写。

XMAX补充独立经纪商公司行动日期佐证，限定两天查询映射后整票1934/1934日期认证通过；
这是隔离认证，不是正式发布，也不是重新认证全量5295只。TEAD仍缺70日、BGMS/CYCC190日、
STEX/BSGM313日。正式coverage/PIT仍09-10；实际09-14候选验证仍因需要09-11而失败。
发布侧逐票降级尚未实现，不能通过忽略三条错误使部分市场冒充完整PASS；详细设计边界见审计。

本轮重新抓取UAN/IBTA/WBI两个分钟间隔。09-11的12个既有唯一事件中，UAN9和IBTA1仍双接口
缺失；WBI09:35桶的一分钟仅一行，量120、high/close32.77，原生五分钟量453、high/close33.12；
09:40桶一分钟无行，原生五分钟量497。已证明响应不一致，但未独立证明时间标签语义和实时
可用性，因此历史分类保持NO_TRADE_CONFIRMED=0、PROVIDER_GAP_CONFIRMED=0、UNRESOLVED=12。
未混源、未合成OHLCV、未回填历史验收；诊断现在按股票和交易日分组，保留逐桶差异。

03:28正式status：v3仍0/5，通过日期为空，最近五日仍09-04/08/09/10/11，失败08/11、缺失04/09/10。
09-11候选600，周期70/78=89.7436%，评估2800，命中0/拒绝2243/等待480/不可评估77/错误0；
可评估53/56=94.6429%、缺口3/56=5.3571%，仍FAIL；P95=0.588805ms、最大77根。
70批合同完整，前八原因和配置85%周期/5%错误/95%可评估/5%缺口/250ms/96根门槛仍按38节，
四张v3表行数及SHA全部未变。运维站继续保留09-11 FAIL和版本，不被09-14 SCHEDULED覆盖。
资源见SG57。发送仍false；解除阻断后仍需至少五个未来连续完整合格交易日，5/5仅供人工验收。

## 41. 2026-09-13：批准的逐票隔离已实现，未部署正式环境

用户批准的0.1%且不超过10只容错已在本地main实现，覆盖coverage发布、跨日继承、显式恢复、
PIT、候选合同和运维显示。完整名单/分母保留，坏股票整票不进入新版本可用行情；缺失日期、
原因、首次隔离日、最后认证版本及源证据随manifest保存，不拼旧价格、不自动解除隔离。
仅允许明确的历史日期缺失，身份/价格语义/hash/未知错误仍阻断，必要基准缺失也阻断。
新增隔离、解除隔离均须使PIT重建并绑定同一可用性合同，候选不得包含隔离股票。

本轮SG真实冻结源重新校验，三只满足分类和额度：完整预期范围8026，隔离3只=0.0373785%。
5295是之前受影响的重认证范围，不是此分母。TEAD冻结认证缺88日（另查18日未拼接，故此前
“仍缺70日”属另一口径），BGMS190、STEX313；未请求供应商，未伪造行情，四张live表SHA未变。
该只读探针不是全量发布验收；正式coverage仍09-10，发送false、v3仍0/5，不补历史失败日。
分钟95%可评估/5%缺口、周期85%、错误5%、P95 250ms、最大96根和五日验收门槛全部未改。
代码与隔离测试、受控发布步骤见[逐票隔离实现记录](../reviews/2026-09-13-security-isolation/README.md)。
最终352项回归通过，26个已测试源码/配置/测试文件与本地SHA一致；本轮未部署、提交或push。

## 42. 2026-09-13：受控部署与正式恢复验收

41节的未部署状态已被本轮上线取代：26个文件经原版本/hash校验部署SG，生产回归202项通过；
随后有界分区重建修复经61项隔离测试、真实5292份历史逐列对照和8项生产测试后部署。
完整准备5295/5295结束，5292只认证通过，TEAD/BGMS/STEX按明确历史缺失隔离，分母8026不变。
XMAX已在本次完整合同中通过1934日期认证。首轮正式发布遇到数千文件查询展开瓶颈，受控停止，
未推进指针；改成每批25文件装载临时数据库，7868781行约41.454秒，首月65082行逐列完全一致。
发布重试仍完整复验冻结范围、禁止新供应商请求，不借性能修复放宽价格、合同或质量门槛。

12:30:51 SGT正式coverage已恢复到09-11：`a5ea8408daa04e4da15735d6790154c5`，93分区、10514109行，
全部发布检查通过，5713/5742=99.494949%目标日覆盖；8026预期股票中8023有可用历史，三只整票
不可用且保留账本，状态PUBLISHED_DEGRADED。12:33:47同版本PIT正式发布
`5c6090a2d5464a429af75147cfd3d992`，完整历史覆盖门禁通过，当前成员2846，三只隔离ID不入成员。
276条隔离资格记录全部false/UPSTREAM_SECURITY_ISOLATED，coverage/PIT/候选availability hash一致。

12:39:56已保存09-14候选，源日期09-11；2844/2846=99.929726%精确日线覆盖，茶杯柄v3筛选2844、
合格1240、选入600，无隔离股票。共享快照外层仍是既有动量容器，但茶杯柄子合同明确为
daily-cup-5m-handle-shadow-v3/2026-09-01.1，不以外层legacy版本计数。此次是提前准备，不是
周一定时器或真实盘中运行的证明。候选artifact SHA为
`3d96fcb2e8297ac20abd43a3e18ecbca7c9f60806e405a4cd3a90cbac4c7d0a3`。

最终四张live表SHA与部署前完全一致。最近五个完整XNYS日09-04/08/09/10/11仍0/5，通过日期为空，
失败08/11、缺失04/09/10。09-11候选600、70/78周期、2800次评估，命中0/拒绝2243/等待480/
不可评估77/错误0；可评估53/56=94.6429%、缺口3/56=5.3571%，P95=0.588805ms、最大77根。
唯一缺口12，NO_TRADE_CONFIRMED=0、PROVIDER_GAP_CONFIRMED=0、UNRESOLVED_SOURCE_GAP=12；
前八原因仍HANDLE_TOO_SHALLOW1793、INSUFFICIENT_COMPLETED_5M_BARS302、HANDLE_VOLUME_NOT_CONTRACTING289、
RIM_NOT_BROKEN156、STALE_QUOTE92、UNRESOLVED_5M_SOURCE_GAP77、HANDLE_NOT_FORMED49、NO_COMPLETED_5M_BARS36。
IBTA两次零突破量仍INSUFFICIENT_VOLUME_EVIDENCE、无信号及有效比例。UAN/IBTA缺证据、WBI接口差异
仍须独立解决，不改历史分类。发送false；仍需至少五个未来连续完整合格交易日，5/5仅报人工验收。
部署后MDB回放2日110次、最大78根、P95=0.875848ms，零信号/误报率null，不是0%，不计入验收。
既有NKTR STOP_FIRST/VTS零量未决后续代理保留，不宣称整体0%误报。运维API候选SUCCESS，盘中
DEGRADED/completed_session保留09-11 FAIL及v3；宽基总任务旧失败和未重建的因子链另行保留。详情见
[受控上线记录](../reviews/2026-09-13-security-isolation/ROLLOUT.md)。

## 43. 2026-09-13：分钟接口差异机制与恢复边界

13:37至13:40 SGT在SG完成29次有界只读请求：24次stable单日/七天/重复对照、4次较早日单独查询，
1次legacy请求403后停止。UAN/IBTA/WBI/SPY的原始响应经现网解析器零丢行、零重复；三种查询的
09-11数据逐值一致，三只问题股票也与03:02保存的六份数据一致，本次未观察到后续修订。
不是简单解析丢失；整数/浮点序列化导致的hash差异已排除，不误报为行情修订。

只比较五根真实、对齐、正且有限OHLCV构成的完整桶：IBTA共8桶全部与native 5min不一致；
WBI40桶中33桶不一致；SPY390分钟完整、78桶中29桶不一致。故缺分钟不能解释全部差异。
13种相对时间偏移测试均不能消除差异；SPY/WBI仅零偏移有完整OHLCV匹配，不能靠改时区或
起止标签直接修复。供应商内部成交筛选、覆盖与修订机制尚无独立证据，不断言哪一种间隔正确。

另确认一个可恢复的历史加载问题：请求09-04至09-11，一分钟仅返回09-09起的数据；单独查询09-08
却返回UAN55/IBTA311/WBI271/SPY390行。分日请求能恢复被宽查询省略的历史日期，但09-11当日
缺口仍在，不能把这一恢复误称为盘中缺口修好。尚未修改生产请求策略，需评估限额及启动耗时。

12个既有缺口仍UAN9/IBTA1双间隔无行，WBI09:35一分钟量120对五分钟453，09:40一分钟无行而
五分钟量497。全部仍UNRESOLVED_SOURCE_GAP，另外两类均0；不混源、不补bar、不回写历史PASS。
当前没有经验证的当日缺口原地恢复路径。若继续评估native 5min，必须独立盘中取证、明确数据版本
及完成/修订/成交量合同，不能将事后native存在等同于盘中可用。发送false、0/5及原门槛不变。
11项隔离诊断测试通过，四张live表与27份生产源码hash未变；运维仍保留09-11 FAIL，不被新候选覆盖。
完整实验与72份证据见[分钟机制取证](../reviews/2026-09-13-cup-resilience/MINUTE_MECHANISM.md)。

## 44. 2026-09-13：下个交易日同期取证已安排，尚未产生实时结果

已部署独立旁路采样器，未修改现网行情或算法。XNYS计划为09-14 21:29:45 SGT开始，覆盖整日，
收盘后至09-15 05:00继续复查，09-15 20:00再次查询09-14数据。UAN/IBTA/WBI/SPY共318轮加次日1轮、
计划2552请求，串行限流、无自动重试；两只具体日期timer已enabled/active，不补跑过期采样。
记录请求/响应时间、双接口串行时间差、原始响应SHA、首次出现/首次有效、修订及消失；分别检验
分钟与五分钟的起始/结束标签假设。稳定观测不是最终不可修订证明，不以native存在直接补洞。

15项SG测试及严格只读沙箱下8次历史冒烟请求通过。冒烟仅用09-11，和未来真实盘中数据隔离，
不计入验收。只允许写专用审计目录，CPU20%/内存硬限256MiB，内存不足300MiB或磁盘不足1GiB则跳过；
权限/配额拒绝、连续错误、时钟异常触发停止。live/recheck状态分开保留，次日成功不覆盖盘中失败。
当前实时请求数仍0，v3仍0/5、发送false，四张历史表未改，运维09-11 FAIL保留。单个旁路heartbeat
安排09-15 05:15及20:15跟进实际结果；不自动换源或开启发送。完整运行说明见
