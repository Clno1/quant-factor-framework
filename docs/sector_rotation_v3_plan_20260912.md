...
1. **SG 实盘（本环境做不到）**：P0 三个交易日观察、成交额 6 标的抽样、17 ETF 持仓活样本、研究/运行口径对齐后的 RS20 对照。本环境也没有已发布的 `US_EQUITY_COVERAGE` 湖，热力图在此会显示覆盖未发布。
...
1. **合入后的 SG 实盘**：§8 第 1、5、6 条。没有这三条，P0–P2 只能称「实现存在」。
2. **P5.2 之前先对齐口径**：运行快照用拆股复权 close×volume；研究脚本下载仍是分红复权 complete OHLCV。对齐前不做 RS20 对照，更不能把广度/净申赎塞进排序。
3. **P5.1 FRED 独立开题**：许可、`known_at`、ALFRED 修订政策写进审计文档之后才能取数。不能用 Yahoo 或 P4.3 的 ETF 代理冒充官方宏观。本环境现在也不具备这条数据链。
4. **P3 维持空列**，直到份额/NAV 有文档化来源。

呈现层到此收口。下一步默认不是继续堆图，而是让已经写下的发布闭环和数据口径在 SG 上跑起来。
