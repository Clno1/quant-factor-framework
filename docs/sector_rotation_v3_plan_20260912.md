...
状态：**开发依据文档**。P0–P3 在分支 `cursor/sector-rotation-v3-99b0`（PR #7）有实现，但**不能视为已完成验收**。P4.0（同组排名路径 / 持续性 / 广度变化，解释字段）已落地；**不得开始 P4.1 轨迹图、P4.2 热力图、P4.3 跨资产条或 P5.1 FRED**。关键闭环、真实量价抽样、17 ETF 活样本与回退演练仍待 SG 兑现。后续板块轮动的迭代严格按本文推进；偏离本文的改动需要先更新本文再实施。
...
| P4.1–4.3 | 未开始 | 相对强弱—速度轨迹、当日热力图、跨资产 ETF 代理条 |
...
代码上 P4.0 已落地。以下仍未做，也不在本步做：

1. **SG 实盘（本环境做不到）**：P0 三个交易日观察、成交额 6 标的抽样、17 ETF 持仓活样本、研究/运行口径对齐后的 RS20 对照。
2. **不要开始** P4.1 散点尾迹、P4.2 热力图、P4.3 跨资产条、P5.1 FRED。
3. P3 审计通过前不拉份额/NAV，也不把广度/净申赎塞进排序。
4. 复查已修的发布实错（盘前 `GROUP_ANALYTICS_ENABLED`、失败会话写成 `latest`、周持仓截断主题缓存、overlay 打断价格层、过期持仓显示「未接入」、关联层污染 replay）不要重做。
