...
状态：**开发依据文档**。2026-09-13 做呈现层收口，阶段性总结见 **§11**。P0–P3 在分支 `cursor/sector-rotation-v3-99b0`（PR #7）有实现，但**不能视为已完成验收**。P4.0–P4.3 已落地；**不得开始 P5.1 FRED**。关键闭环、真实量价抽样、17 ETF 活样本与回退演练仍待 SG 兑现。后续板块轮动的迭代严格按本文推进；偏离本文的改动需要先更新本文再实施。
...
| 轮动 | 同组20日排名、较20日前变化；详情给路径与持续性 | P4.0 解释字段，不进优先级 |
...
**目标**：按交易所实际收盘与供应商就绪时间发布主题价格快照；个股关联失败可重试且未发送摘要可受控换版；页面区分最新、历史、过期与失败。

启动时间、完成 SLA、失败补偿分开写，不得把「17:30 启动」写成「收盘后 1.5 小时内完成」。常规日 16:00 ET 收盘后 17:30 才启动，下载与计算还要时间；提前收盘日更不满足 1.5 小时承诺。
...
`price` 阶段发布前必须先比对 **发布指纹**，覆盖实际输出依赖，不能只比较行情面板：

```text
原始数据指纹     input_fingerprint          主题价量与 execution_close 面板
计算配置指纹     config_fingerprint         schema、parameters、amount_verified、
                                           amount_audit_status、decision_cutoff、
                                           主题定义、背景证据
...
使用 `America/New_York` 让 systemd 自动处理夏令时。**17:30 是启动时间，不是完成时刻。** 完成 SLA 登记为启动后 75 分钟；提前收盘日仍按 17:30 启动，不承诺相对当日实际收盘的固定时差。FMP 凭据通过 `EnvironmentFile=-/etc/quant/market-data.env`（及可选 `momentum-alerts.env`）进入价格层与周持仓进程。

价格层 `MemoryHigh=700M` / `MemoryMax=900M`，因为 `--refresh` 同时刷新主题 canonical 与成员 `holdings_canonical`。

`quant-group-analytics-eod.timer` 保持 13:15 SGT，`ExecStart` 为 `--stage linkage`。停用价格 timer **不会**自动恢复价格构建：13:15 任务已改为只做 linkage。
...
- **同一 `run_id` 必须更新运行健康**：`freshness`、`last_attempt`、有效主题数。报告正文可以保持固定，过期和失败告警必须动态更新。固定 `?run=` 快照不把 latest 的正文合并进来。
...
**验收**：SG 上观察连续 3 个交易日：价格层在 17:30 ET 启动后于登记 SLA 内出快照；至少复现一次「13:15 关联失败 → 07:00 ET 重试成功 → 未发送摘要换版为新结果」。提前收盘日只核对应启动与补偿，不核 1.5 小时承诺。
...
5. 验证：页面能打开且 schema 与读取器匹配；盘前 prepare/send 不引用已下线 run。
6. 旧 run 文件保留、不覆盖，是必要条件但不是完整回退。
...
保留两个不同状态，不能用一个 `verified=true` 混在一起：

- **计算路径**：始终用 canonical `close × volume`；缺少 `close` 时金额为 null，禁止回退 `adj_close`。
- **`amount_verified`**：仅表示生产轨按 canonical 字段解锁「成交活跃」列。
- **`amount_audit_status`**：`SYNTHETIC_GATES_ONLY`（当前）或真实抽样通过后的 `PASSED`。

合成测试只能证明代码乘对了两个字段，不能证明供应商实际返回的价量复权一致。

`scripts/research_group_rotation.py` 仍下载分红复权 complete OHLCV，与运行快照不是同一条计算链。P5 必须先增加「研究计算与运行计算逐值一致」门槛，再谈是否跑赢 RS20。若保留旧口径作对照，应单独命名，不能都叫 `source_v1_compat`。
...
1. 停止 v3 writer。
2. 恢复可读 v2.1 的代码版本与 unit。
3. 把有效 `latest.json` 指回最后一份 v2.1 run（旧读取器会拒绝 v3 schema）。
4. 处理未发送摘要后验证页面与投递。

新代码能读 v2，不等于旧代码能读 v3。schema 双版本可读只覆盖「新读取器看旧快照」。
...
- **持仓名单**按约定频率更新（周一 12:00 ET 周任务，`--no-refresh-members`）。
- **成员股价每日更新**（价格层 `--refresh` 写入 `rotation/holdings_canonical/`，不进主题 `rotation/canonical/`）。
- 日度广度基于「当前名单 × 目标交易日成员价格」组合计算。

周任务完成后，连续两个交易日构建广度必须能得到 `eligible_members>0`（成员缓存不得停在上周五）。历史 `--asof` 不得拿今天最新持仓配过去价格；`point_in_time=false` 只是标签，代码必须拒绝这种组合（`HOLDINGS_NOT_POINT_IN_TIME`）。
...
measured_fund_weight_pct      已测基金权重（有价格的映射成员权重合计）
measurement_complete          样本与权重覆盖是否达到资格
```

主表与 Discord 不能把部分持仓显示成完整广度。必须同时显示：已测基金权重；已映射样本内参与率；`measurement_complete=false` 时标明「仅部分持仓观察」。例如 80% 权重无法映射、剩余 20% 全部站上均线时，不得只写「等权 100%／加权 100% · 样本 5/5」。
...
- **只用于当日展示**。不倒填历史、不进 `validation.py` 的回测样本、不参与任何前向研究标签。`--asof` 早于最新完整交易日时不得叠加当前持仓观测。
- 页面文案同时给出样本内参与率、已测基金权重；覆盖不足时写「仅部分持仓观察」
...
**测试**：双口径同时输出；权重合计越界拒绝；成分映射失败进 `excluded`；持仓观测过期时降级为 `unavailable` 而非使用旧数据；`point_in_time` 恒 false；持仓数据不进 validation 样本；成员价格日更与周名单分离；部分覆盖不得显示为完整广度；历史 asof 不得使用当前持仓。

**验收**：17 个 ETF 全部有真实广度或明确的不可用原因；SMH 结果与试点一致；周任务后连续两个交易日 `eligible_members>0`。
...
- 与价格方向组合成**事实标签**（只写组合及数据日期，不作机构意图判断）：`价格涨+净流入`、`价格涨+净流出`、`价格跌+净流入`、`价格跌+净流出`
- 「含金量高 / 反弹出货 / 逢低吸纳」属于待检验假设，不进计划正文与产品文案。ETF 申赎可能涉及实物交换、套利和做空安排，不能据此识别机构意图。
- 这些标签不是评分项，不进优先级
...
**目标**：把已有数据变成可读的产品。**不新增任何算法，不新增总分。**

设计审阅指出：双轴能标出近期强弱，但要看出「轮动」需要**排名迁移、持续性 vs 单日脉冲、广度变化**，而不是再做一个总分。因此 P4 先做解释时间线（P4.0），再做散点尾迹（P4.1）。热力图与跨资产条仍按原顺序排在后面。

#### P4.0 同组排名路径与持续性（先于轨迹图）

同一观察范围、同一窗口、同一基准（科技 vs QQQ，行业 vs SPY，禁止混排）：

- `rank_rs20` / `rank_rs20_n` / `rank_rs20_ago20` / `rank_change20`（20日前位次 − 今日位次，正数=名次改善）
- `rank_path20`：T−20 / T−15 / T−10 / T−5 / 今日的同组 RS20 位次
- `outperform_days20`：近 20 个交易日中 `rs1 > 0` 的天数
- `log_share_5_of_20`：`ln(1+RS5/100) / ln(1+RS20/100)`；≥ 0.5 视为近端少数日贡献为主
- `persistence`：`gradual` 逐步跑赢 / `event_spike` 近期少数日贡献为主 / `repair` 反弹未收复 / `fading` 20日仍领先但近端掉队 / `lagging` 近20日落后 / `unavailable` 不足
- 篮子主题用生产历史的 `breadth_now` / `breadth_ago20` / `breadth_change20`
- ETF 持仓观测只有当前点：`breadth_source=etf_holdings_current_only`，**禁止编造历史时点广度**

约束：

- 字段挂在 `row.rotation_timeline`，**不写入 `production`**（replay 仍只比对 production / compatibility）
- **不改** `assign_priority`，不把广度/净申赎塞进排序
- 主表「轮动」列只显示紧凑位次；详情与 Discord 给路径/持续性
- 旧 v2 快照没有该字段时页面必须降级为「—」

#### P4.1 相对轮动轨迹图

X/Y 已在快照中（`strength_log` / `acceleration_log`），`history` 已存 120 个交易日。主表 API 不回传完整 history，因此另存紧凑 `rotation_trail`（默认最近 50 根，约 10 个交易周），**不写入 production**。

- 散点 + 尾迹，默认最近 10 周，可切 5 周
- 四象限底色，随观察范围（科技/行业）切换，禁止混排
- 点击圆点打开同一主题详情
- **命名约束**：JdK RS-Ratio / RS-Momentum 公式为专有。本图命名为"相对强弱—速度轨迹"，文案明确"借鉴相对趋势与轨迹表达，非 RRG 复刻"
- 不改 `assign_priority`，不进总分

#### P4.2 板块热力图

数据源：`US_EQUITY_COVERAGE`（约 3000+ 只，含行业分类与市值），我们的底层覆盖优于 Finviz。

- 当日市值加权 treemap，按 sector / sub-industry 两级下钻
- 周期切换：1 日 / 5 日 / 20 日
- 点击下钻到个股，复用既有 `/breakouts/{ticker}` 链接
- **约束**：行业分类当前为 `LATEST_KNOWN_BACKFILL_NOT_PIT`。做**当日**热力图合规（只需今天的分类），必须页面标注，且**不得用于历史回看**
- 读已发布的 `US_EQUITY_COVERAGE`，不在 Web 请求里打行情供应商；不写入 `production`，不改 `assign_priority`
...
- P4.0 只依赖 P1 已有的 `history` / `rs20` / `rs1` / 篮子广度；P4.1 轨迹图额外需要 `strength_log` / `acceleration_log`。P4.2 热力图读已发布 `US_EQUITY_COVERAGE`，不依赖 P2/P3 验收，也不得用于历史回看。
...
1. **SG 上线后的 P0 观察**：核价格层是否在 17:30 ET **启动**并在登记 SLA 内完成；至少复现一次「13:15 关联失败 → 07:00 ET 重试成功 → 未发送摘要换版」。需只读 `systemctl` / `journalctl` / `outputs/group_analytics/rotation/last_attempt.json`。不把「收盘后 1.5 小时内完成」当验收标准。
...
5. **P1 成交额 6 标的实盘抽样**：合成门禁已绿，`amount_audit_status` 仍为 `SYNTHETIC_GATES_ONLY`；3 只有拆股、3 只有大额分红的 FMP 抽样仍待 SG
6. **P2 17 个 ETF `/etf/holdings` 活样本**与「周名单后连续两日日度广度」集成验收：本环境无 FMP key
...
8. **P5 研究/运行口径对齐**：`make_panel` 已按 close×volume 计算；`research_group_rotation.py` 下载仍是分红复权 complete OHLCV。对齐前不得把研究积分与运行快照都叫当前 `source_v1_compat` 去比 RS20。广度/净申赎在对照跑赢前不得进排序
...
| 2026-09-12 | 外部审阅（对照 6297897）：日度成员价格、amount 双状态、发布指纹、部分覆盖展示、关联换版、父版本校验、轮询健康、运维适配与回退步骤未兑现。实现补齐后仍不视为 P0–P3 验收通过，不得直接开 P4.1/P5 |
| 2026-09-13 | P4.0：同组 RS20 排名路径、持续性与广度变化作为解释字段落地；不改优先级；不拉 FRED、不做热力图/轨迹图 |
| 2026-09-13 | P4.1：相对强弱—速度轨迹（散点+尾迹，非 RRG 复刻）；不改优先级；不做热力图/跨资产/FRED |
| 2026-09-13 | P4.1 轨迹图按卡片宽度绘图，窄屏用图例避免标签重叠；仍不进优先级 |
| 2026-09-13 | P4.2：当日市值加权 treemap（coverage，非 PIT）；固定历史快照不绘图；不进优先级；不做跨资产/FRED |
...
| P4.0 | **实现存在** | 同组排名路径 / 持续性 / 广度变化；解释字段，不进优先级；ETF 不编造历史广度 |
| P4.1 | **实现存在** | 相对强弱—速度轨迹；散点+尾迹；非 RRG；不进优先级 |
| P4.2 | **实现存在** | 当日市值加权 treemap；LATEST_KNOWN 分类；历史回看禁用；不进优先级 |
| P4.3 | **实现存在** | 六个 ETF 代理 5/20/60 日绝对收益条；标 ETF 代理；历史快照仍可显示；不进优先级 |
| P5 | 未开始 | 先对齐研究与运行口径，再谈 RS20；FRED 独立审计 |

总体设计（双轴强弱、独立风险、广度不强行控制排名）保留，不需要推倒重来。工具定位是日线研究地图，不是已验证的轮动交易策略，也不是盘中实时轮动。

### 10.2 本轮已修的发布与数据链缺口

1. 价格层 `--refresh` 同时刷新 `holdings_canonical` 成员股价；周持仓任务改为 `--no-refresh-members`。
2. `amount_audit_status` 与 `amount_verified` 分离；缺少 close 时金额为 null。
...
- P4.0 排名路径与持续性不进优先级
- P4.1 轨迹图不进优先级
- P4.2 当日热力图不进优先级，也不用于历史回看
- P4.3 跨资产条不进优先级；文案只称 ETF 代理，不称美元指数或黄金/原油现货
- 周持仓任务不进 `operations.yaml`（周日历 vs `latest_publishable_xnys` 会误报 MISSED）
- 盘前报告 `kind` 即使在 v3 也必须走 `rotation_payload`
- 资金流标签只写事实组合，不写机构意图

### 10.4 下一步

呈现层在代码上已经收口，完整对照见 **§11**。以下仍未做，也不在本步做：
...
## 11. 阶段性总结（2026-09-13，呈现层收口）

这一轮 = 计划文档 + PR #7 上的 P0–P3 实现与复查 + P4.0–P4.3 呈现层。  
**停点**：呈现层代码已齐，可以给人看路径、轨迹、当日板块和跨资产背景。  
**不是停点**：P0–P3 验收、真实数据链、对 RS20 的增量验证、FRED。

原文目标仍有效：

> 先让它成为一个可靠、可核对、信息不互相覆盖的板块研究工具，再讨论预测能力。

本轮没有增加评分复杂度，也没有把轮动结论变成自动选股或仓位依据。`promotion` 仍是 `NOT_APPROVED`。

### 11.1 对照原问题，现在改善了什么

§1 的三个症状对应五类根因。代码上已经动刀的部分：

| 原问题 | 本轮做了什么 | 还缺什么 |
|---|---|---|
| 以为数据滞后；页面停在打开时的快照；Discord 链到历史 run 看不出是历史 | 价格层与关联层拆开；latest 轮询 freshness / last_attempt；钉住 `?run=` 时标明固定历史快照 | SG 上三个交易日观察 17:30 启动与关联失败→07:00 重试→未发送摘要换版 |
| 「真实广度」「研究优先级」大量空白或「等待确认」 | 双轴把「稳定领先」从死区里救出来；等待原因拆成缺口码；ETF 用当前持仓观测，篮子用成员站上 20 日线；过期/非 PIT/部分覆盖分开写 | 17 ETF 活样本未跑；持仓无生效日；3–4 只篮子永远到不了 `breadth_n >= 5` |
| 整体像玩具：看不出轮动，只有一张表 | P4.0 同组排名路径与持续性；P4.1 相对强弱—速度轨迹（非 RRG）；P4.2 当日市值加权 treemap；P4.3 六个 ETF 代理条 | 热力图依赖已发布 `US_EQUITY_COVERAGE`；跨资产条是 ETF 代理不是指数/现货；宏观仍空 |

没有用新数据源去填分数。空白能解释了，不等于数据已经齐。

### 11.2 阶段对照：实现 vs 验收

唯一合入分支仍是 `cursor/sector-rotation-v3-99b0`。原计划「一阶段一 PR、验收后再开下一阶段」已由负责人豁免为同分支连续交付；豁免的是开发门禁，**不是验收**。

| 阶段 | 代码 | 验收 | 一句话 |
|---|---|---|---|
| 计划 | 本文 | 作为开发依据使用中 | 偏离先改本文 |
| P0 时效与闭环 | 有 | 未做 | 17:30 是启动时刻；SG 观察、回退演练未兑现 |
| P1 判断层 | 有 | 未做 | 双轴与独立风险已上主表；成交额仍是 `SYNTHETIC_GATES_ONLY`；0–100 分不在主表 |
| P2 真实广度 | 管线有 | 未做 | 当前持仓观测，不是历史成分；无 17 ETF 活样本 |
| P3 净申赎 | 空列管线 | 审计 NOT_PASSED | 公开文档没有份额/NAV 历史；禁止用成交额冒充 |
| P4.0 排名路径 | 有 | 本环境页面核对 | 解释字段，不进优先级；ETF 不编造 20 日前广度 |
| P4.1 轨迹图 | 有 | 本环境页面核对 | 非 RRG；数值来自已有 `strength_log` / `acceleration_log` |
| P4.2 热力图 | 有 | 合成覆盖交互核对 | 当日、非 PIT；钉住历史快照不绘图；本环境无 coverage 湖则 fail closed |
| P4.3 跨资产条 | 有 | 本环境页面核对 | ETF 代理绝对收益；历史快照仍显示；不进评分 |
| P5.1 FRED | 无 | — | **不要开始** |
| P5.2 增量验证 | 无 | — | 研究下载与运行快照口径未对齐前，不得拿现行 `source_v1_compat` 去比 RS20 |

隔离边界未破：只动 `src/group_analytics/rotation/`、`src/premarket_digest/rotation.py`、轮动 Web、轮动 systemd。多因子 / 回测 / 模拟盘 / 茶杯柄 / EP 未改。

### 11.3 本轮刻意保持、因此不能对外宣称的

- 这是日线研究地图，不是已验证的轮动交易策略，也不是盘中实时轮动。
- 生产 0–100 分全程不在主表；成交额解锁也不恢复总分展示。
- 广度、净申赎、P4 全部解释层都不进 `assign_priority`。
- 不把 UUP 叫美元指数，不把 GLD 叫黄金现货，不把 treemap 当历史行业分类。
- 不把自建对数象限称为 RRG。
- Discord 无新图形、不改排名规则。
- 复查已修的发布实错（周持仓截断 canonical、关联缺口写入 production、盘前 `kind` 走错渲染器等）不要重做。

### 11.4 下一阶段的门（按优先级，不是按愿望）

1. **部署到 SG 之前必须先合 `main`。** `main` 上并行合入了 EP 事件队列消费者验收、茶杯柄影子数据隔离与 coverage 受控修复。本分支若直接上 SG，这些提交会丢。2026-09-13 已把 `origin/main` @ `44d5062` 合进 `cursor/sector-rotation-v3-99b0`（无文件冲突）。之后若 `main` 再往前走，上 SG 前再合一次，不要用过期的分叉去覆盖机器。
2. **合入后的 SG 实盘**：§8 第 1、5、6 条。没有这三条，P0–P2 只能称「实现存在」。
3. **P5.2 之前先对齐口径**：运行快照用拆股复权 close×volume；研究脚本下载仍是分红复权 complete OHLCV。对齐前不做 RS20 对照，更不能把广度/净申赎塞进排序。
4. **P5.1 FRED 独立开题**：许可、`known_at`、ALFRED 修订政策写进审计文档之后才能取数。不能用 Yahoo 或 P4.3 的 ETF 代理冒充官方宏观。本环境现在也不具备这条数据链。
5. **P3 维持空列**，直到份额/NAV 有文档化来源。

呈现层到此收口。下一步默认不是继续堆图，也不是 FRED。先让已经写下的发布闭环和数据口径在 **已包含最新 `main` 的本分支** 上于 SG 跑起来。
