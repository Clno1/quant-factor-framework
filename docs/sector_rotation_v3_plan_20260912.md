...
状态：**开发依据文档**。P0–P3 在分支 `cursor/sector-rotation-v3-99b0`（PR #7）有实现，但**不能视为已完成验收**。P4.0 时间线与 P4.1 相对强弱—速度轨迹已落地；**不得开始 P4.2 热力图、P4.3 跨资产条或 P5.1 FRED**。关键闭环、真实量价抽样、17 ETF 活样本与回退演练仍待 SG 兑现。后续板块轮动的迭代严格按本文推进；偏离本文的改动需要先更新本文再实施。
...
- P4.0 只依赖 P1 已有的 `history` / `rs20` / `rs1` / 篮子广度；P4.1 轨迹图额外需要 `strength_log` / `acceleration_log`。P4 不依赖 P2/P3 验收，但 ETF 历史广度在持仓无 PIT 之前必须保持空。
...
| P4.2–4.3 | 未开始 | 当日热力图、跨资产 ETF 代理条 |
...
代码上 P4.0 与 P4.1 已落地。以下仍未做，也不在本步做：

1. **SG 实盘（本环境做不到）**：P0 三个交易日观察、成交额 6 标的抽样、17 ETF 持仓活样本、研究/运行口径对齐后的 RS20 对照。
2. **不要开始** P4.2 热力图、P4.3 跨资产条、P5.1 FRED。
